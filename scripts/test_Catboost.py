...
def train_and_cv(df, feature_cols, category_cols, cat_params, model_name, save_dir, scaler_file=None, n_splits=5):
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf_split):
        ...
        joblib.dump(cat_model, model_file)
        
        print(f"Fold {fold+1} Trainning finished. Time elapsed: {time()-training_start:.2f}s")
        
        y_pred_valid = cat_model.predict(df_valid_fold_2[feature_cols])
...
if __name__ == "__main__":
    ...
    model_name = f"cat_no_ta_add_daily_{len(feature_cols)}"
    ...
    save_dir = f"/home/lishi/projects/Competition/kaggle_2023/data/cat_models/{model_name}"
